...
const Invoice = ({ onNavigateToInvoice, totalCartPrice }) => {
  ...
  useEffect(() => {
    ...
    const newProducts = cartItems.map((product) => {
      ...
      return {
        ...
        NameProduct_AR: product[0].ProductName_ar,
  ...
  const handleSubmit = async (e) => {
    ...
    formData.BillDetiles.forEach((product, index) => {
      ...
      form.append(`BillDetiles[${index}].NameProduct_AR`, product.NameProduct_AR);
  ...
  return (
    <div className={style.invoice_container}>
      ...
      <form onSubmit={handleSubmit}>
        <label>
          {t('Your Name:')}
          <input type="text" placeholder={t('Enter your Name...')} name="CustomerName" value={formData.CustomerName} onChange={handleChange} />
        </label>
        <label>
          {t('Your Phone:')}
          <input type="text" placeholder={t('Enter your Phone...')} name="CustomerPhone" value={formData.CustomerPhone} onChange={handleChange} />
        </label>
        <label>
          {t('Your Email:')}
          <input type="email" placeholder={t('Enter your Email...')} name="CustomerEmail" value={formData.CustomerEmail} onChange={handleChange} />
        </label>
        <label>
          {t('Your Address:')}
          <input type="text" placeholder={t('Governorate / city / district')} name="CustomerAdderes" value={formData.CustomerAdderes} onChange={handleChange} />
        </label>
        <h5 className='text-danger'>
          {t('Order Total:')} {formData.totalprice}
        </h5>
        <button type="submit" disabled={!isFormValid}>
          {t('Submit your request')}
        </button>
        {showErrorMessage && <p>{t('Please fill out all fields.')}</p>}
      </form>
    </div>
  );
};
